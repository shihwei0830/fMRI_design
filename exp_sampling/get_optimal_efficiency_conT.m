...
% exceeding nT_run.
%
% UB_yes: 1= add std of ISI distribution and ITI distribution to the mean
% ISI and ITI for upper bound; 0 = mean of ISI and ITI distribution as
% upper bound
%
% 
%
% results = get_optimal_efficiency_conT(40,600,[2 8;2 20],2,[4 6],20000,5,'exp3_2',1)
%
%
...
if length(mu)>1
    for i=1:length(mu)
        ...
        if UB_yes
            sd_sample(i) = std(sample);
        end
    end
    if UB_yes
        t_isi_total = ceil(t_jitter_total*(new_mu(1)/sum(new_mu)) + sd_sample(1))
        t_iti_total = ceil(t_jitter_total*(new_mu(2)/sum(new_mu)) + sd_sample(2))
    else
    ...
    if UB_yes
        ...
        t_isi_total = ceil (t_jitter_total/2);
    end        
...
count_n_sim_iti = 0;
while 1
     t_iti_0 = get_expo_sample(t_range(2,:),tr,mu(2),n_trials);
